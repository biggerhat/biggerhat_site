            ...
            <div class="min-h-0 m-0 mx-auto bg-gray-300 rounded-lg w-min">
                <div class="mx-auto card__image">
                    <div class="w-full h-full card__image--front">
                        <img src="\storage\{{ $mini->cards[$currentCard]->front }}"
                            class="w-full h-full rounded-lg cursor-pointer" />
                    </div>
                    <div class="w-full h-full card__image--back">
                        <img src="\storage\{{ $mini->cards[$currentCard]->back }}"
                            class="w-full h-full rounded-lg cursor-pointer" />
                    </div>
                </div>
            </div>
            ...
            <div class="mb-2 bg-gray-200 border-2 border-black rounded shadow-md">
                ...
                <div class="mx-2 mb-2">
                    ...
                    <p class="pb-1 mb-1 text-xl font-semibold">Keyword(s): @foreach ($mini->keywords as $keyword)<a href="/keywords/{{ $keyword->slug }}"
                                class="inline-block text-center p-0.5 text-white rounded-full {{ $background }} px-2 py-1 text-sm font-bold mr-3">{{ strtoupper($keyword->name) }}</a>
                        @endforeach</p>
                    <div class="block my-1 border-b border-gray-400 border-dashed"></div>
                    <p class="pb-1 mb-1 text-xl font-semibold">Characteristic(s): @foreach ($mini->characteristics as $characteristic)<span
                                class="inline-block text-center p-0.5 text-white rounded-full {{ $background }} px-2 py-1 text-sm font-bold mr-3">{{ strtoupper($characteristic->name) }}</span>
                        @endforeach</p>
                </div>
            </div>
            ...
            <div class="mb-2 bg-gray-200 border-2 border-black rounded">
                ...
                <div class="mx-2 mb-2">
                    @foreach ($relateds as $relatedMini)
                        <a href="/characters/{{ $relatedMini->slug }}"
                ...
                <div class="bg-gray-200 border-2 border-black rounded">
                    ...
                    <div class="mx-2 mb-2">
                        @foreach ($mini->promos as $promo)
                            <a href="/promos/{{ Str::slug($promo->name, '-') }}"
                                class="inline-block text-center p-0.5 text-white rounded-full {{ $background }} px-2 py-1 text-xs font-bold mb-1 cursor-pointer">{{ strtoupper($promo->name) }}</a><br />
                        @endforeach
                    </div>
                </div>
            </div>
