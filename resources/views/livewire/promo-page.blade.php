...
<div>
    <div
        ...
        <div class="mx-1 mb-3 sm:my-3 md:m-0 xl:col-span-3 lg:col-span-2 md:col-span-2">
            <div class="mb-2 shadow-lg">
                <div
                    class="pane-title {{ getBackground($promos[$currentCard]->mini) }}">
    ...
    <div class="container grid mx-auto auto-cols-fr lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 gap-x-2">
        ...
        @endif
        @if (count($promos[$currentCard]->cultures) > 0)
            <div class="mx-1 mb-1">
                <div class="shadow-lg">
                    <div class="pane-title {{ getBackground($promos[$currentCard]->mini) }}">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" style="height: 32px; width: 32px;"
                             class="inline-block">
                            <g class="" transform="translate(0,0)" style="">
                                <path
                    ...
                    <div class="pane-body">
                        @foreach ($promos[$currentCard]->cultures as $culture)
                            @if($culture->image)
                                <a href="{{ $culture->link }}" target="_new"><img src="/storage/{{ $culture->image }}"
                                                                                  class="mx-auto mb-2 border border-gray-500 rounded"></a>
                            @endif
                            <a href="{{ $culture->link }}" target="_new"
                               class="mb-2 hover:underline font-bold">{{ $culture->name }}</a> @if($culture->description)
                                - {{ $culture->description }}@endif
                        @endforeach
                    </div>
                </div>
            </div>
